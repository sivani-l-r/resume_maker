...
export const resumePDF = (personalData = {}, formData = { education: [] }) => {
    ...
    formData.projects.forEach((proj) => {
        ...
        startY += 5;
        setFont(doc, 'times', 'normal', 12);
        addText(doc, `${proj.desc || ''}`, 10, startY); 
        const textDimensions = doc.getTextDimensions(`${proj.desc}`);
        const textHeight = textDimensions.h; 
        startY += textHeight + 18; 
    });
    ...
    formData.extracurriculars.forEach((extra) => {
        ...
        startY +=5;
        setFont(doc,'times','normal',12);
        addText(doc, `${extra.desc}`, 10,startY);
        const textDimensions = doc.getTextDimensions(`${extra.desc}`);
        const textHeight = textDimensions.h; 
        // startY += textHeight + 5; 
    });
